...
```text
c-shell 0.3.0  ·  cc (GCC) 16.1.1 (默认标准 gnu23)
In [1]: int x = 41;
In [2]: x + 1
Out[2]: 42
In [3]: 3 / 2
...
- **标准库索引。** `%where` 可查询 ISO C 标准库名称所属的标准头文件及其
  标准版本信息。
- **终端、浏览器与脚本。** 可直接使用 REPL，也可在本机浏览器中获得相同的
  PTY 交互体验，或使用 `-e`、脚本文件和管道输入，并获得确定的退出状态。
...
c-shell --web                        # 在本机打开浏览器终端
c-shell --web --no-open              # 只显示本机网址，不自动打开
...
`c-shell --web` 会在伪终端中启动普通交互式 REPL，并在默认浏览器中打开
xterm.js 终端。因此补全、颜色、多行编辑、Ctrl-C，以及 `scanf` 等程序输入
都沿用终端行为，而不是另写一套 Web 版 REPL。

服务器始终在 `127.0.0.1` 的随机端口监听，刻意不提供局域网或公网监听选项。
每次启动还会生成新的 128 位随机 URL 路径，只接受来自该本机页面精确 Origin
的 WebSocket 升级，并使用仓库内置资源而非 CDN。每个浏览器标签页会创建独立
的 c-shell 进程；关闭标签页会终止对应进程，在启动它的终端按 Ctrl-C 会停止
服务器及其活动会话。

如只需显示网址，可使用 `--web --no-open`。编译器、语言标准、超时、界面语言、
静默模式和颜色选项会传递给每个浏览器会话。`--web` 不能与 `--eval` 或
`--script` 同时使用。随机 URL 是额外的本机防护，不应把它分享给不可信的
本机进程或用户。
